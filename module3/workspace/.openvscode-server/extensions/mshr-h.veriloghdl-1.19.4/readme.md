...
2. Open the repository in VS Code and start the default build task (`watch`) or simply press `F5`—the `Launch Extension` configuration in [.vscode/launch.json](https://github.com/mshr-h/vscode-verilog-hdl-support/blob/HEAD/.vscode/launch.json) will run the build task automatically.
