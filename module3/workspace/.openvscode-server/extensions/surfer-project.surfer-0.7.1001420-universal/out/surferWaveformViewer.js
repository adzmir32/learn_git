...
function openDialogMessage(kind, fileUri, webview) {
        ...
        webviewPanel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'vscodeSaveStateFromWasm': {
                    ...
                    const suggestedFileName = typeof message.fileName === 'string' && message.fileName.trim().length > 0
                        ? message.fileName.trim()
                        : 'surfer_state.surf.ron';
                    const defaultUri = vscode.Uri.file(require('path').join(require('os').homedir(), suggestedFileName));
